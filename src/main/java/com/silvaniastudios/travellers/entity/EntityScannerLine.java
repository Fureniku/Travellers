...
public class EntityScannerLine extends Entity {
	...
	/**
	 * Whether this entity should be killed immediately, or should wait for NBT data first
	 */
	...
	/**
	 * @param hitEntity the hitEntity to set
	 */
	...
	public RayTraceResult getLookedAt(float partialTicks) {
		...
		if (player != null) {
			if (this.world != null) {
				...
				Vec3d playerLookExtended = playerEyeVector.addVector(playerLook.x * reachDistance, playerLook.y * reachDistance, playerLook.z * reachDistance);
				pointedEntity = null;
				Vec3d vec3d3 = null;

				List<Entity> closeEntityList = world.getEntitiesInAABBexcluding(
						player, player.getEntityBoundingBox().expand(playerLook.x * reachDistance, playerLook.y * reachDistance, playerLook.z * reachDistance)
								.grow(1.0D, 1.0D, 1.0D),
				...
				for (int j = 0; j < closeEntityList.size(); ++j) {
					...
					RayTraceResult playerEyeLookIntercept = entityBoundingBox.calculateIntercept(playerEyeVector, playerLookExtended);
