...
public class GuiSchematicInfoScreen extends GuiScreen {
	...
	public GuiSchematicInfoScreen(ItemStack stack) {
		if (stack.getItem() instanceof ItemSchematic) {
			...
			this.descText = I18n.format("travellers.tooltip." + this.schem.getName());
	...
	public void initGui() {
		...
		this.middleRepeat = Math.floorDiv(fontRenderer.getWordWrappedHeight(descText, xSize - 16), bottomSection - middleSection) + 1;
		...
		this.buttonList.add(new GuiButton(1, this.guiLeft + 82, this.guiTop + bottomSectionStart + 8, 60, 20, "Salvage"));
	}
	...
	public void drawBackground(int tint) {
		...
		for (int i = 1; i <= this.middleRepeat; i++) {
			this.drawTexturedModalRect(this.guiLeft, this.guiTop + (middleSection * i), 0, middleSection, 150, bottomSection);
		}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.fontRenderer.drawString(I18n.format(this.stack.getItem().getUnlocalizedName() + ".name"), this.guiLeft + 38,
				this.guiTop + 12, 5592405);
		
		this.fontRenderer.drawSplitString(I18n.format("travellers.tooltip." + this.schem.getName()), this.guiLeft + 8,
				this.guiTop + 40, xSize - 16, 5592405);
	}
