...
public class Travellers {
	...
	public void preInit(FMLPreInitializationEvent event) {
		...
		System.out.println("Doing preinit");

		/*RenderingRegistry.registerEntityRenderingHandler(EntityScannerLine.class,
			new IRenderFactory<EntityScannerLine>() {
		...
		// RenderingRegistry.registerEntityRenderingHandler(EntityScannerLine.class,
		// new RenderScannerLine(Minecraft.getMinecraft().getRenderManager()));
		proxy.preInit(event);
	}
	...
	public static class RegistrationHandler {
		...
		public static void registerEntities(RegistryEvent.Register<EntityEntry> event) {
			int id = 0;
			//EntityEntry scannerLineEntry = EntityEntryBuilder.create().entity(EntityScannerLine.class)
			//		.id(new ResourceLocation(Travellers.MODID, "scanner_line"), id++).name("scanner_line")
			//		.tracker(64, 20, false).build();
			
			EntityEntry scannerLineEntry = EntityEntryBuilder.create()
					.entity(EntityScannerLine.class)
					.factory(EntityScannerLine.EntityScannerFactory::createEntityScanner)
					.id(new ResourceLocation(Travellers.MODID, "scanner_line"), id++).name("scanner_line")
					.tracker(64, 20, false).build();

			event.getRegistry().register(scannerLineEntry);
