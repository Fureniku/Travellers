...
public class EventHandler {
	...
	public void onPlayerLogsIn(PlayerLoggedInEvent event) {
		...
		// player.getCapability(KnowledgeProvider.KNOWLEDGE, null);

		IPlayerData playerData = player.getCapability(PlayerDataProvider.PLAYER_DATA, null);

		String message = String.format("%sYou have %d knowledge%s", TextFormatting.GOLD,
				playerData.getKnowledgeBalance(), TextFormatting.RESET);
		player.sendMessage(new TextComponentString(message));

		String message1 = String.format("%sYou are entity {%s}%s", TextFormatting.GOLD, player.getUniqueID().toString(),
